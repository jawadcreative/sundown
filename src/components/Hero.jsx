...
const Hero = () => {
  ...
  return (
    <section ref={sectionRef} className="md:h-screen h-[60vh] w-full relative">
      ...
      <div className="absolute z-20 h-full w-full px-[2vw] pt-[3vw] flex items-end border-b">
        <Navbar />
        <div className="md:h-[65vh] h-[40vh] w-full flex md:flex-row flex-col-reverse items-center md:justify-between justify-around">
          <p
            ref={paraRef}
            className="font-gilroy md:text-[2.5vw] md:w-[42%] w-full leading-none">
            Sundown is a multi-disciplinary studio focused on creating unique, end-to-end experiences and environments.
          </p>

          <div ref={headRef} className="flex flex-col items-end max-md:w-full">
            {["SPACES", "THAT", "INSPIRE"].map((item, i) => (
              <h1
                key={i}
                className="md:text-[10vw] text-[60px] leading-none font-gilroy"
              >
                {item}
              </h1>
            ))}
